...
function getInitialColorScheme() {
  ...
  const storedColorScheme = localStorage.getItem('color-scheme')?.trim();
  let colorScheme;

  if (/^(light|dark)$/.test(storedColorScheme)) {
...
function LightDarkButton() {
  const [mode, setMode] = useState(getInitialColorScheme);
  ...
  return (
    <button className="light_dark_button" onClick={handleClick}>
      <svg
        ...
        fill="currentColor"
      >
        {mode === 'dark' && <LightIconPath />}
        {mode === 'light' && <DarkIconPath />}
      </svg>
    </button>
  );
}
